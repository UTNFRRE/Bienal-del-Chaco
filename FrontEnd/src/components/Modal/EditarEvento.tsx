...
export default function ModalEditarEvento({
  ...
  const handleconfirmar = () => {
    confirmar(
      ...
      descripcion,
      fecha,
      longitud ?? -58.981256488503035,
      latitud ?? -27.43757052684891
    ); // -58.981256488503035, -27.43757052684891 son las coordenadas de Resistencia, Chaco
    onClose();
  };
    ...
    if (e.key === 'Enter') {
      ...
      if (address) {
        const response = await fetch(
          `https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(address)}&components=locality:Resistencia|administrative_area:Chaco|country:AR&key=${googleMapsApiKey}`
        );
